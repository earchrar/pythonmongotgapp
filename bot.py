...
async def getattcode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Fail to record attendance.")
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        records = list(
            collection.find({"user_id":user_id}).sort("created_At",pymongo.ASCENDING) # -1
        )
        ...
        if records:
            ...
            for record in records:
                ...
                created_Atstr = record.get("created_At").strftime("%Y-%m-%d %H:%M:%S") if created_At else "N/A"
                message += (
                    f"Class: {record.get('class')}\n"
                    f"Student ID: {record.get('studentid')}\n"
                    f"Attendance Code: {record.get('attcode')}\n"
                    f"Date: {created_Atstr}\n\n"
                )
        else:
